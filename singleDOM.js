...
function init(){
    ...
    domEvents.addEventListener(sphere[0], 'click', event => {
        tl.to(sphere[0].scale , 1 , { z: 2, x: 2, y: 2}).to(sphere[0].rotation , 1 , { x: 60}, "-=1").to(sphere[0].rotation , .7 , { x: 0.01}).to(sphere[0].scale , 1 , { z: 1, x: 1, y: 1},"-=.7")
    })
    domEvents.bind(sphere[1], 'click', function(){          // .bind ... function() == .addEventListener... event =>
        tl.to(sphere[1].scale , 1 , { z: 2, x: 2, y: 2})
    })
    domEvents.bind(sphere[1], 'click', function(){
        tl.to(sphere[1].scale , 1 , { z: 1, x: 1, y: 1})
    })
    ...
    domEvents.bind(sphere[3], 'click', function(){
        tl.to(sphere[3].scale , 1 , { z: 2, x: 2, y: 2})
    })
    domEvents.bind(sphere[3], 'click', function(){
        tl.to(sphere[3].scale , 1 , { z: 1, x: 1, y: 1})
    })
