...
var pointLight= new THREE.PointLight(0x00FF00, 1, 1)
...
function onMouseMove(event) {
    ...
    for(var i = 0; i< intersects.length; i++){
        tl = new TimelineMax()
        tl.to(intersects[i].object.rotation, 1,{z: offset, x: offset}).to(intersects[i].object.position, 1, {x: Math.random() * 100 - 50, y: Math.random() * 100 - 50}, "-=1")
    }
    tl.to(intersects[i].object.material.color.set(0x00FF00))
    offset += Math.PI/2;
    if (offset > 2*Math.PI){
        offset = Math.PI/2
    }
